...
public interface UserRepository extends PagingAndSortingRepository < UserEntity, Long > {
 ...
 UserEntity findByUserId(String userId); // veritabanında user_id olarak saklanıyor ama burada _ yokmus gibi
 // kullanıyoruz.
 ...
 @Query(value = "select * from Users u where u.first_name = ?1", nativeQuery = true) // ?1 ilk parametreyi al demek
 List < UserEntity > findUserByFirstName(String firstName);

 @Query(value = "select * from Users u where u.last_name = :lastName", nativeQuery = true) // lastName @param ile
 // match olmalı
 ...
 @Query("select user from UserEntity user where user.userId =:userId") // JPQL. veritabanı tablo yada sütun ismini
 // degil java entity classındaki
 // tanımlamalarla olusturuyoruz ifademizi.
